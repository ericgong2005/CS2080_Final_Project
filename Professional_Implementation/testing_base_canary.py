...
class TransformerLanguageModel(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            # temperature adjustment for testing
            scaled_logits = logits / temperature 
            # apply softmax to get probabilities
            probs = F.softmax(scaled_logits, dim=-1) # (Batch x Channels)
...
start_string = "The secret code is"

context = torch.tensor(encoder(splitter(start_string)), dtype=torch.long, device=device).unsqueeze(0)

generation = full_decode(decoder(m.generate(context, max_new_tokens=100, temperature=0.5)[0].tolist()))
